...
let globalMatchType = "partial" // Set default to "partial"
...
export default function ManualSearch() {
  ...
  }, [searchTerm, selectedRarity, selectedNode, matchType, sortColumn, sortDirection])

  const filteredData = useMemo(() => {
    // Don't filter if no search criteria are provided
    if (!searchTerm && !selectedRarity && !selectedNode) return []
  ...
  }, [marketData, searchTerm, selectedRarity, selectedNode, matchType, sortColumn, sortDirection])
  ...
  return (
    <Card className="bg-gray-800 border-gray-700">
      <CardHeader>
        <CardTitle className="text-amber-500">Manual Search</CardTitle>
        <CardDescription className="text-gray-300">Search for items by name, rarity, and node</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {marketData.length > 0 && filteredData.length === 0 && (searchTerm || selectedRarity || selectedNode) && (
